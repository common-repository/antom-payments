...
const CardPayFields = (props) => {
    ...
    const handleExpireDateChange = (event) => {
        ...
        if (isDeleteMod && need_delete) {
            value = value.slice(0, value.length - 1)
        }

        value = value.replace(/^0+/, '')

        if (value == '' && !isDeleteMod) {
            value = '0'
        }

        let format_value = ''
        let temp_value = ''
        let has_fixed_zero = false
        for (let i in value) {
            if (i == 0 && !['0', '1'].includes(value[i])) {
                temp_value = '0' + value[i]
                has_fixed_zero = true
            } else {
                temp_value = value[i]
            }

            if (i == 1 && value[i] >= 2 && !has_fixed_zero) {
                temp_value = 2
            }

            if (format_value.length < 4) {
                format_value = format_value + temp_value
            }
        }


        let month_text = format_value.slice(0, 2)
        let year_text = format_value.slice(2)

        let final_value = ''

        if (month_text) {
            final_value = month_text
        }
        ...
        if (year_text.length > 0) {
            final_value += year_text
        }

        setExpireDate(final_value);
            ...
            <div className="antom-cards">
                ...
                <div className="antom-cards-container">
                    <div className="antom-cards-container-item">
                        <div className="antom-cards-container-item-line">
                            ...
                            <input placeholder="Card number" className="antom-card-number" value={cardNumber} onChange={handleCardNumberChange} ref={cardNumberInputRef} />
                        </div>
                ...
                <div className="antom-cards-container">
                    <div className="antom-cards-container-item expire-date">
                        <div className="antom-cards-container-item-line">
                            <input placeholder="Exipre date" className="antom-expire-date" value={expireDate} onChange={handleExpireDateChange} ref={expireDateInputRef} />
                        </div>
                        <div className="antom-cards-container-item-line antom-cards-container-item-error antom-expire-date-error"></div>
                    </div>
                    <div className="antom-cards-container-item">
                        <div className="antom-cards-container-item-line">
                            <input placeholder="CVC" className="antom-cvc" value={cardCvc} onChange={handleCardCvcChange} ref={cvcInputRef} />
                        </div>
                ...
                <div className="antom-cards-container">
                    <div className="antom-cards-container-item">
                        <div className="antom-cards-container-item-line">
                            <input placeholder="Holder name" className="antom-holder-name" value={cardHolderName} onChange={handleCardHolderName} ref={holderNameInputRef} />
                        </div>
    ...
    return (
        <div>
            {isLoggedIn ? <FormFields /> : <WarningUserNotLogin settings={settings} />}
        </div>
