...
abstract class WC_Gateway_Antom_Common extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		if ( $payment_request_checker->payment_request_ids_number_is_limit() ) {
			return $this->return_failure(
				__(
					'The maximum number of payment orders has been reached.',
					'antom-payments'
				)
			);
		}
		...
		$client = new Antom_Alipay_Client( $api_host, $private_key, '' );
		...
		if ( $is_card_payment_method ) {
			if ( in_array(
				...
				$order->update_status( 'on-hold', sprintf( __( 'request payment by antom_payment_gateway card payment, payment request id is : %1$s, %2$s ', 'antom-payments' ), $payment_request_id_info['payment_request_id'], $mode_note ) );
				WC()->cart->empty_cart();
				...
				return $this->inquiry_card_payment_request_status(
					$payment_request_checker,
					$payment_request_id_info['payment_request_id'],
					$response_body['result']->resultMessage,
					$order_view_url,
					$order
				);
			}
			...
			if ( in_array(
				...
				// update order to on-hold and add note
                $order->update_status( 'on-hold', sprintf( __( 'request payment by antom_payment_gateway %1$s,  payment request id is : %2$s, %3$s ', 'antom-payments' ), $payment_method_type, $payment_request_id_info['payment_request_id'], $mode_note ) );
                WC()->cart->empty_cart();
				$response['result'] = 'success';
	...
	private function handler_order_with_payment_capture_notify(
		...
		if ( 'PAYMENT_RESULT' === $notify_type ) {
			if ( 'F' === $params['result']['resultStatus'] ) {
				...
				$this->set_order_pay_failed( $order, $payment_status, $payment_request_id );
			}
		} elseif ( 'CAPTURE_RESULT' === $notify_type ) {
	 ...
	 * @return void
	 */
	protected function set_order_pay_failed( $order, $payment_status, $payment_request_id ) {

		// If the order is unpaid, set it to fail.
		if ( ! ( $order->is_paid() || $order->get_status() === 'cancelled') ) {
			$order->update_status( 'failed', sprintf( __( 'update by antom_payment_gateway, payment request id is : %1$s. ', 'antom-payments' ), $payment_request_id ) );
		}
