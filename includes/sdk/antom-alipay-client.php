...
class Antom_Alipay_Client {
	...
	protected function sendRequest( $request_url, $http_method, $headers, $request_body ) {
		...
		foreach ( $headArr as $headerItem ) {
			if ( ! is_array( $headerItem ) && ( strstr( $headerItem, 'response-time' ) || strstr( $headerItem, 'signature' ) ) ) {
