	...
	if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
		...
		foreach ( $payment_methods as $payment_method ) {
			if ( Antom_Payment_Gateways_Statement::ANTOM_DEVELOP_MODE || in_array( $currency, $payment_method['support_currencies'] ) ) {
				...
				if ( 'antom_card' === $payment_method['slug'] ) {
					add_action( 'wp_enqueue_scripts', 'antom_load_card_checkout_block_event', 100 );
				}
			}
		}
	}
...
function antom_load_card_checkout_block_event() {
	wp_enqueue_script(
		'antom-card-checkout-events',
		ANTOM_PAYMENT_GATEWAYS_URL . 'assets/js/card-events.js',
		array( 'wc-checkout-block-frontend' ),
